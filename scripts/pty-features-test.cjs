...
(async () => {
  ...
  note('command event with duration', !!dur, dur ? `exit=${dur.exitCode} duration=${dur.durationMs}ms` : `saw ${commands.length} events`);
  note('duration is plausible (>=900ms)', !!dur && dur.durationMs >= 900, dur ? `${dur.durationMs}ms` : 'n/a');
  ...
  note('history includes session commands', all.includes('echo HISTORY-ONE'), `${all.length} entries`);

  ws.send(JSON.stringify({ type: 'history', prefix: 'git', limit: 50, requestId: 'r2' }));
  await sleep(1200);
  const git = history && history.items ? history.items : [];
  note('prefix search is filtered', git.length > 0 && git.every((c) => c.startsWith('git')), JSON.stringify(git.slice(0, 3)));

  ws.send(JSON.stringify({ type: 'history', prefix: 'echo', limit: 50, requestId: 'r3' }));
  await sleep(1200);
  const echoes = history && history.items ? history.items : [];
  note('prefix search excludes other commands', echoes.length > 0 && echoes.every((c) => c.startsWith('echo')), `${echoes.length} echo entries`);

  const failed = checks.filter((c) => !c.ok);
  console.log('\n=== shell integration features ===');
  for (const c of checks) console.log(`${c.ok ? 'PASS' : 'FAIL'}  ${c.name.padEnd(38)} ${c.detail}`);
