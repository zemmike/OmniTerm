...
### Fixed
- **AI Reader with Claude Code on Windows.** The welcome box, pixel-art logo, input-box
  placeholder and footer no longer collapse into one unreadable paragraph. Before Claude
  has answered, the reader says so; after, it shows only the answer.
- **AI Reader filters** show clearly when they are on (filled, with a tick).
