...
export const SettingsView: React.FC = () => {
  ...
  const field = 'w-full bg-[#0D0D0F] border border-[#2A2A2E] rounded px-2.5 py-1.5 text-xs text-[#E0E0E5] focus:outline-none focus:border-[var(--ui-accent)] focus-visible:ring-1 focus-visible:ring-[var(--ui-accent)]';
    ...
    <div className="p-4 md:p-5 space-y-4 text-[#E0E0E5] overflow-y-auto" style={{ height: 'calc(100vh - 120px)' }}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          {saved && (
            <span role="status" aria-live="polite" className="text-[10px] text-[var(--ui-accent)] flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          {settings.theme === 'custom' && (
            <div className="grid grid-cols-2 gap-3 pt-1">
              {(
                ...
                <div key={key}>
                  <label className={label} htmlFor={`settings-color-${key}`}>{text}</label>
        ...
        <div className={section}>
          ...
          <div>
            <label className={label} htmlFor="settings-font">Font</label>
            <select id="settings-font" value={settings.fontFamily} onChange={(e) => set('fontFamily', e.target.value)} className={field}>
              {FONT_STACKS.map((f) => (
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className={label} htmlFor="settings-font-size">Size ({settings.fontSize}px)</label>
              <input
            ...
            <div>
              <label className={label} htmlFor="settings-line-height">Line height</label>
              <input
            ...
            <div>
              <label className={label} htmlFor="settings-scrollback">Scrollback</label>
              <input
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={label} htmlFor="settings-cursor">Cursor</label>
              <select id="settings-cursor" value={settings.cursorStyle} onChange={(e) => set('cursorStyle', e.target.value as TerminalSettings['cursorStyle'])} className={field}>
            ...
            <label className="flex items-end gap-2 text-xs text-[#8A8A93] pb-1">
              <input type="checkbox" checked={settings.cursorBlink} onChange={(e) => set('cursorBlink', e.target.checked)} className="accent-[var(--ui-accent)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ui-accent)]" />
              Blinking cursor
            </label>
          </div>
          <div
                      ...
                      return (
                        <div key={action.id} className="flex items-center gap-2 text-[11px]">
                          ...
                          <button
                            ...
                            aria-label={isRec ? `Recording a shortcut for ${action.label} — press the keys now` : `Record a shortcut for ${action.label}`}
                            aria-pressed={isRec}
